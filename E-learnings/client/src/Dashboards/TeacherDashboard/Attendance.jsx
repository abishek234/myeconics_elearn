...
const Attendance = ({ userId }) => {
    ...
    const fetchStudents = async () => {
        ...
        try {
            const response = await axios.get(`http://localhost:9000/api/teachers/teacher/class/students?classid=${classId}&schoolname=${schoolname}`);
    ...
    const handleCheckboxChange = (rollNo) => {
        // Toggle the attendance status for the specific student
        ...
        const periods = [
            {
                ...
                subject: subject,
                presentStudents: Object.keys(attendanceData).filter(rollNo => attendanceData[rollNo]), // Collect roll numbers that are marked present
                absentStudents: students.map(student => student.rollno).filter(rollNo => !attendanceData[rollNo]) // Collect roll numbers that are marked absent
            }
        ];
                                    ...
                                    <Grid item xs={12} sm={6} md={4} key={student._id}>
                                        <Card>
                                            <CardContent>
                                                ...
                                                <Typography variant="h6">{student.rollno}</Typography>
                                                <FormControlLabel
                                                    control={
                                                        <Checkbox
